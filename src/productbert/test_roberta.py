...
def main(config):
    ...
    with torch.no_grad():
        for i, inputs in enumerate(tqdm(data_loader)):
            data, attn_mask, target, target_multi1, target_multi2 = inputs['input_ids'].to(device), inputs['attention_mask'].to(device), inputs['labels'].to(device), inputs['label_multi1'].to(device), inputs['label_multi2'].to(device)

            target = target.transpose(0, 1)
            target_multi1 = target_multi1.transpose(0, 1).squeeze()
            target_multi2 = target_multi2.transpose(0, 1).squeeze()

            output_binary, output_multi1, output_multi2 = model(data, attn_mask)
